...
if [ -x "$SCRIPT_DIR/s3-lfs" ]; then
  LFS_S3_BIN="$SCRIPT_DIR/s3-lfs"
elif [ -x "$REPO_ROOT/s3-lfs" ]; then
  LFS_S3_BIN="$REPO_ROOT/s3-lfs"
elif [ -x "$REPO_ROOT/packages/cli/test/s3-lfs" ]; then
  LFS_S3_BIN="$REPO_ROOT/packages/cli/test/s3-lfs"
else
  echo "Could not find s3-lfs binary. Build it first, e.g.: go build -o s3-lfs ./packages/cli/cmd/s3-lfs"
  exit 1
fi
...
echo "# This is a s3-lfs test." > README.md
git add README.md
...
git config --add lfs.customtransfer.s3-lfs.path "$LFS_S3_BIN"
git config --add lfs.customtransfer.s3-lfs.args '--access_key_id='"$AWS_ACCESS_KEY_ID"' --secret_access_key='"$AWS_SECRET_ACCESS_KEY"' --bucket='"$S3_BUCKET"' --endpoint='"$AWS_S3_ENDPOINT"' --root_path='"$ROOT_PATH"
...
git lfs install --local
git config --add lfs.customtransfer.s3-lfs.path "$LFS_S3_BIN"
git config --add lfs.customtransfer.s3-lfs.args --root_path="$ROOT_PATH"
git config --add lfs.standalonetransferagent s3-lfs
git config --add lfs.concurrenttransfers 2
git reset --hard main
git lfs pull
...
cd local-repo-dup
git config --replace-all lfs.customtransfer.s3-lfs.args '--access_key_id='"$AWS_ACCESS_KEY_ID"' --secret_access_key='"$AWS_SECRET_ACCESS_KEY"' --bucket='"$S3_BUCKET"' --endpoint='"$AWS_S3_ENDPOINT"' --compression=gzip --root_path='"$ROOT_PATH"
git lfs push --all origin main
cd ..
